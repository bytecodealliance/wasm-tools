...
impl TestCase for MyInterpreter {
    fn call_export(wit: Wit, func: Function, args: OwnVals<'_, Self>) -> Option<Box<Val>> {
...

        {
            ...
            assert_eq!(ret, Some(Box::new(Val::ByteList(Vec::new()))));
            ...
            assert_eq!(ret, Some(Box::new(Val::ByteList(b"abc".to_vec()))));
        }

        {
            ...
            assert_eq!(ret, Some(Box::new(Val::GenericList(Vec::new()))));
            let ret = Self::call_import(
                wit,
                Some("a:b/x"),
                "echo-u32",
            ...
            assert_eq!(
                ret,
                Some(Box::new(Val::GenericList(vec![
                    Box::new(Val::U32(1)),
                    Box::new(Val::U32(2)),
                ])))
            );
...

        {
            ...
            let ret = Self::call_import(
                ...
                "echo-string",
                &[Val::GenericList(vec![
                    Box::new(Val::String("1".to_string())),
                    Box::new(Val::String("2".to_string())),
                ])],
            );
            assert_eq!(
                ret,
