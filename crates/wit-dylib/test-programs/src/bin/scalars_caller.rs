...
impl TestCase for MyInterpreter {
    fn call_export(wit: Wit, func: Function, args: OwnVals<'_, Self>) -> Option<Box<Val>> {
        ...
        assert_eq!(ret, Some(Box::new(Val::F32(1.))));
        ...
        assert_eq!(ret, Some(Box::new(Val::F64(1.))));
        ...
        assert_eq!(ret, Some(Box::new(Val::Bool(false))));

        let ret = Self::call_import(wit, Some("a:b/x"), "echo-char", &[Val::Char('x')]);
        assert_eq!(ret, Some(Box::new(Val::Char('x'))));
