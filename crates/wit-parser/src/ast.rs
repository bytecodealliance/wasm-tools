...
pub struct SpanLocation {
    /// File to which this span belongs
    pub file: String,
    /// UTF-8 byte offet within the file (start)
    pub start: usize,
...
{err}
 ...
 {line:4} | {snippet}
    ...
    pub fn get_location(&self, span: Span) -> Option<SpanLocation> {
        if !span.is_known() {
            return None;
        }
        let start = span.start();
        ...
        Some(SpanLocation {
            file: src.path.clone(),
            start: src.to_relative_offset(start),
            end: src.to_relative_offset(end),
        })
    }
