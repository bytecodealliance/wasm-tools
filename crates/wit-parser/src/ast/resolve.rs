        ...
        Ok(match ty {
            ...
            ast::Type::Map(map) => {
                ...
                if !is_valid_key {
                    bail!(Error::new(
                        map.span,
                        format!("invalid map key type: map keys must be bool, u8, u16, u32, u64, s8, s16, s32, s64, char, or string")
                    ));
                }
    ...
    fn find_stability(&self, kind: &TypeDefKind, stability: &Stability) -> Stability {
        ...
        fn find_in_kind<'a>(
            ...
            match kind {
                ...
                TypeDefKind::Map(k, v) => find_in_type(types, *k)
                    .or_else(|| find_in_type(types, *v)),
