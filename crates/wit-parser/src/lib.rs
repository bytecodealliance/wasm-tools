...
pub(crate) fn place_setters_after_getters<K, V>(
    ...
    func: impl Fn(&V) -> Option<&Function>,
) {
...
mod test {
    ...
    fn test_canon_version_split() {
        ...
        assert_eq!(
            ...
            ("1".to_string(), Some(".2.3".to_string()))
        );

        let v = Version::parse("101.201.301").unwrap();
        assert_eq!(
            PackageName::version_compat_track_string(&v),
            ("101".to_string(), Some(".201.301".to_string()))
        );

        let v = Version::parse("0.2.6-rc.1").unwrap();
        assert_eq!(
            PackageName::version_compat_track_string(&v),
            ("0.2.6-rc.1".to_string(), None)
        );

        let v = Version::parse("0.10.0+build.1").unwrap();
        assert_eq!(
            PackageName::version_compat_track_string(&v),
            ("0.10".to_string(), Some(".0+build.1".to_string()))
        );

        let v = Version::parse("0.0.1-alpha+build.1").unwrap();
        assert_eq!(
            PackageName::version_compat_track_string(&v),
            ("0.0.1-alpha".to_string(), Some("+build.1".to_string()))
        );
        ...
        assert_eq!(
            PackageName::version_compat_track_string(&v),
            ("1.0.0-beta.1".to_string(), None)
        );

        let v = Version::parse("0.0.100+build.1").unwrap();
        assert_eq!(
            PackageName::version_compat_track_string(&v),
            ("0.0.100".to_string(), Some("+build.1".to_string()))
        );
    }
}
