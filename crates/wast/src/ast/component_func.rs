    ...
    fn parse(parser: Parser<'a>) -> Result<Self> {
        parser.parens(|parser| {
            ...
            let (ty, kind) = if let Some(import) = parser.parse()? {
                ...
                let ty = parser.parse()?;
                let locals = ComponentLocal::parse_remainder(parser)?;
                (
                    ty,
...
pub struct ComponentLocal<'a> {
    ...
    pub ty: ast::InterType<'a>,
}

impl<'a> ComponentLocal<'a> {
    pub(crate) fn parse_remainder(parser: Parser<'a>) -> Result<Vec<ComponentLocal<'a>>> {
        ...
        while parser.peek2::<kw::local>() {
            parser.parens(|p| {
                ...
                locals.push(ComponentLocal { id, name, ty });
                while parse_more && !p.is_empty() {
                    locals.push(ComponentLocal { id: None, name: None, ty: p.parse()? });
                }
                Ok(())
            })?;
        }
