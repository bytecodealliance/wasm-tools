            ...
            match  p {
                $(
                    $tpe => {
                        let options = [
                            $(  
                                move | config, chunk, out, payload  | $mutation.mutate(config, chunk, out, payload), // Instantiate after select
                            )*
                        ];
                ...
                Payload::CodeSectionEntry(reader) => {
                    ...
                    for i in ops_reader.into_iter() {
                        ...
                        match i.unwrap() {
                            ...
                            Operator::End => assert_eq!(0, 0),
                            _ => {
                                panic!("Other instruction than unreachable should not be possible for this function")
                            }
                        }
                    }
                },
