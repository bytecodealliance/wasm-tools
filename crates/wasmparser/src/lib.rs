...
macro_rules! for_each_operator {
    ($mac:ident) => {
        $mac! {
            ...
            @mvp Block { blockty: $crate::BlockType } => visit_block (arity enter -> enter)
            @mvp Loop { blockty: $crate::BlockType } => visit_loop (arity enter -> enter)
            @mvp If { blockty: $crate::BlockType } => visit_if (arity 1 enter -> enter)
            @mvp Else => visit_else (arity else -> else)
            ...
            @legacy_exceptions Try { blockty: $crate::BlockType } => visit_try (arity enter -> enter)
            @legacy_exceptions Catch { tag_index: u32 } => visit_catch (arity leave -> tag)
            @legacy_exceptions Rethrow { relative_depth: u32 } => visit_rethrow (arity 0 -> 0)
            @legacy_exceptions Delegate { relative_depth: u32 } => visit_delegate (arity leave -> leave)
            @legacy_exceptions CatchAll => visit_catch_all (arity leave -> 0)
            @mvp End => visit_end (arity else leave -> else leave)
            ...
            @tail_call ReturnCall { function_index: u32 } => visit_return_call (arity func ret -> func)
            @tail_call ReturnCallIndirect { type_index: u32, table_index: u32 } => visit_return_call_indirect (arity 1 type ret -> type)
            ...
            @function_references ReturnCallRef { type_index: u32 } => visit_return_call_ref (arity 1 type ret -> type)
