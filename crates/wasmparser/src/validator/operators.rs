    ...
    R: WasmModuleResources,
{
    ...
    fn maybe_exact_type(&self, ty: MaybeType) -> bool {
        match ty {
            MaybeType::Known(ValType::Ref(rt)) if rt.is_exact_type_ref() => true,
    ...
    fn pop_concrete_or_exact_ref(
        ...
        Ok((ty, self.maybe_exact_type(ty)))
    }
    ...
    fn check_ref_cast_desc(&mut self, nullable: bool, heap_type: HeapType) -> Result<()> {
        ...
        let (_, is_exact) = self.pop_concrete_or_exact_ref(true, descriptor_idx)?;
        if let HeapType::Exact(_) = heap_type {
            if !is_exact {
                bail!(
                    self.offset,
                    "type mismatch: expected exact type because the descriptor is exact"
                );
            }
        }
    ...
    T: WasmModuleResources,
{
    ...
    fn visit_struct_new(&mut self, struct_type_index: u32) -> Self::Output {
        ...
        if self.features.custom_descriptors() {
            self.push_exact_ref(false, struct_type_index)?;
        } else {
    ...
    fn visit_struct_new_default(&mut self, type_index: u32) -> Self::Output {
        ...
        if self.features.custom_descriptors() {
            self.push_exact_ref(false, type_index)?;
        } else {
    ...
    fn visit_ref_get_desc(&mut self, type_index: u32) -> Self::Output {
        let (_ty, is_exact) = self.pop_concrete_or_exact_ref(true, type_index)?;
    ...
    fn visit_br_on_cast_desc(
        ...
        let descriptor_idx = self.check_descriptor(to_ref_type.heap_type())?;
        ...
        let (_, is_exact) = self.pop_concrete_or_exact_ref(true, descriptor_idx)?;
        if to_ref_type.is_exact_type_ref() && !is_exact {
            bail!(
                self.offset,
                "type mismatch: expected exact type because the descriptor is exact"
            );
