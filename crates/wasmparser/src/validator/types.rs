...
impl TypeInfo {
    ...
    /// of a type size.
    #[cfg(feature = "component-model")]
    pub(crate) fn combine(&mut self, other: TypeInfo, offset: LogicalOffset) -> Result<()> {
...
impl TypeList {
    ...
    pub fn intern_sub_type(&mut self, sub_ty: SubType, offset: LogicalOffset) -> CoreTypeId {
    ...
    pub fn intern_func_type(&mut self, ty: FuncType, offset: LogicalOffset) -> CoreTypeId {
    ...
    pub(crate) fn reftype_is_subtype_impl(
        ...
        let core_type_id = |group: Option<RecGroupId>, index: UnpackedIndex| -> CoreTypeId {
            if let Some(id) = index.as_core_type_id() {
                ...
                self.at_canonicalized_unpacked_index(group.unwrap(), index, LogicalOffset::MAX)
