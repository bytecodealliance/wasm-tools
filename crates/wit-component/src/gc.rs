...
pub fn run(
    wasm: &[u8],
    required: &IndexSet<String>, // cabi_import_realloc is the only alloc-related routine in here.
    ...
    module.liveness()?; // Walks around and determines, using adapter's exports as roots, what needs to be kept. So it would seem realloc_via_malloc would have to be added before this, lest malloc get culled. [No, malloc is in the victim module.] It seems the caller passes in `required`, which it claims has any needed realloc function in it. Prove it by experiment. [It has only cabi_import_realloc.] Or maybe have the caller add "malloc" to that list. Or have this add it.
    // √ And maybe do malloc import before we start keeping track of realloc_index.
    // This code does NOT, as of before I messed with it, add an export for any of the reallocs. So maybe they're in the raw wasm? [The adapter alone imports (__main_module__, cabi_realloc), calls it only from State::new(), and exports cabi_import_realloc.] Maybe look at an adapted module. [Main doesn't import or export anything it didn't initially. Adapter exports cabi_import_realloc (which uses its own bump allocator). realloc_via_memory_grow() is there, called from State::new and allocate_stack. I don't see anything clearly call cabi_import_realloc, but it gets aliased, and it's hard to trace what might reference that.]
    // So it seems like whatever realloc_via_* is made at adaptation time (as is readily apparent here).
    // The wasip1 adapter tries to import cabi_realloc from main. If it's not there, this gins one up.
    module.encode(realloc_scheme)
    // The realloc_via_* is called only from State::new and allocate_stack.
}
...
impl<'a> Module<'a> {
    ...
    fn encode(&mut self, realloc_scheme: ReallocScheme) -> Result<Vec<u8>> {
        ...
        // Adds an import of a function with type `type_idx` from the main
        // module to the adapter. Updates associated bookkeeping. Returns the
        // index of the newly imported function.
        //         let mut import_function_from_main = |name: &str, type_idx: u32| -> u32 {
        //             // Tell the function remapper we're reserving a slot for our extra import:
        //             map.funcs.next += 1;
        //
        //             imports.import("__main_module__", name, EntityType::Function(type_idx));
        //             func_names.push((num_func_imports, name));
        //
        //             let func_idx = num_func_imports; // TODO: Make sure all this various bookkeeping is needed at every callsite.
        //             num_func_imports += 1;
        //             func_idx
        //         };
        ...
        if let (Realloc(realloc_name), Some(_), None) = (&realloc_scheme, sp, realloc_index) {
            ...
            // let index = import_function_from_main(realloc_name, add_realloc_type(&mut types));
            // realloc_index = Some(index);
                    ...
                    let realloc_func = match realloc_scheme {
                        Malloc(_) => {
                            self.realloc_via_malloc(
                                //import_function_from_main(malloc_name, add_malloc_type(&mut types)),
        ...
        if sp.is_some() && (realloc_index.is_none() || allocation_state.is_none()) {
            ...
            // function instead.
            realloc_index = Some(num_func_imports + funcs.len());
            funcs.function(add_realloc_type(&mut types)); // TODO: In every case, make sure my addition of a malloc import doesn't shift the function indices. (Function indices first count imports, then local funcs. May need to keep separate offsets and add them unless I can put all import additions first.) [I moved all the imports to the top, so this shouldn't be a problem now.]
