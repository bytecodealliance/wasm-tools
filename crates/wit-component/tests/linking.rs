use {
    ...
    wit_parser::{Resolve, UnresolvedPackage, UnresolvedPackageGroup},
};
...
fn encode(wat: &str, wit: Option<&str>) -> Result<Vec<u8>> {
    ...
    if let Some(wit) = wit {
        ...
        let UnresolvedPackageGroup { mut packages, source_map } = UnresolvedPackage::parse(Path::new("wit"), wit)?;
