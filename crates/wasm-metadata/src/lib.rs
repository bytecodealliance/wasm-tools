    ...
    pub fn from_binary(input: &[u8]) -> Result<Self> {
        ...
        for payload in Parser::new(0).parse_all(&input) {
            match payload? {
                ...
                ModuleSection { unchecked_range: range, .. } => metadata.push(Metadata::empty_module(range)),
                ComponentSection { unchecked_range: range, .. } => metadata.push(Metadata::empty_component(range)),
                End { .. } => {
