...
impl WitOpts {
    ...
    fn run(self) -> Result<()> {
        ...
        let output = match wit_component::decode(name, &bytes).context("failed to decode world")? {
            DecodedWasm::WitPackage(resolve, package) => {
                ...
                let doc = match &self.document {
                    ...
                    None => match pkg.documents.len() {
                        ...
                        _ => bail!(
                            "more than document found in package, specify \
                             which to print with `-d name`"
                        ),
                    },
                };

                printer.print(&resolve, doc)?
            }
            // TODO
            DecodedWasm::Component(..) => panic!(),
        };
