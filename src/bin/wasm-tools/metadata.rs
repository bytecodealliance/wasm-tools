...
impl<'a> ModuleNames<'a> {
    ...
    fn from_reader(section: NameSectionReader<'a>) -> Result<ModuleNames<'a>> {
    ...
    fn add_opts(&mut self, opts: &AddOpts) {
        self.module_name = opts.name.clone();
    }
    fn get_name(&self) -> Option<&String> {
        self.module_name.as_ref()
    }
    fn into_section(self) -> Result<wasm_encoder::NameSection> {
...
impl<'a> ComponentNames<'a> {
    ...
    fn from_reader(section: ComponentNameSectionReader<'a>) -> Result<ComponentNames<'a>> {
        let mut s = Self::empty();
        for name in section.into_iter() {
            let name = name?;
            match name {
    ...
    fn add_opts(&mut self, opts: &AddOpts) {
        self.component_name = opts.name.clone();
    }
    fn get_name(&self) -> Option<&String> {
        self.component_name.as_ref()
    }
    fn into_section(self) -> Result<wasm_encoder::ComponentNameSection> {
...
/// Read and add metadata (module name, producers) to a WebAssembly file.
#[derive(clap::Parser)]
pub enum Opts {
    Show(ShowOpts),
    Add(AddOpts),
            ...
            match payload? {
                Version { encoding, .. } => {
                    if metadata.is_empty() {
                        match encoding {
                            wasmparser::Encoding::Module => metadata.push(Metadata::module()),
                            wasmparser::Encoding::Component => metadata.push(Metadata::component()),
                        }
                    }
                }
                ModuleSection { .. } => metadata.push(Metadata::module()),
                ComponentSection { .. } => metadata.push(Metadata::component()),
                End { .. } => {
                    let finished = metadata.pop().expect("non-empty metadata stack");
                    if metadata.is_empty() {
                        write!(output, "metadata: {finished:?}")?;
                    } else {
...
impl Output {
    fn section(&mut self, section: &(impl wasm_encoder::Section + wasm_encoder::ComponentSection)) {
        match self {
            Output::Component(c) => {
                c.section(section);
            }
    ...
    fn as_slice(&self) -> &[u8] {
        match self {
            Output::Component(c) => c.as_slice(),
            Output::Module(m) => m.as_slice(),
        }
    }
}
